...
# 結果が残らないときは警告にとどめる。
#
# 包んだアプリは、ビルドし直した直後の初回起動で Python（約 100MB）の展開から
# 始まり、環境によっては数分待っても Python 側が動き出さない。
# Windows の CI でも同じことが起きている。**落ちた項目があるときに止める**のが
# ここの役目なので、結果が出ないことを理由に配布を止めない。
if [[ ! -f "$report" ]]; then
  print -u2 "警告: 自己診断が結果を残しませんでした（起動が間に合っていない可能性）"
  exit 0
fi
