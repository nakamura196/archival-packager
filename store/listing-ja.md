...
画面は日本語と英語。**ただしマニフェストは `ja-JP` だけを宣言している**
（`packaging/windows/AppxManifest.xml.in`）。英語の画面を入れたのに申告が
追いついていない状態で、`en-US` を足すには MSIX を作り直すことになる。
掲載情報のテキストだけでは直らないので、次に版を出すときに合わせること。

## 販売する国・地域

全世界。**英語の掲載情報（en-us）はまだ無い**ので、英語圏の利用者にも
日本語の説明文が出る。画面自体は英語になる。
