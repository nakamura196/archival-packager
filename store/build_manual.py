...
para("")
for line in [
    "このアプリは、デジタル資料の長期保存（OAIS 参照モデル）のための情報パッケージを",
    "作成する業務用ツールです。学術研究の成果物であり、無料で提供します。",
    "",
    "同梱している第三者コンポーネントについて:",
    "",
    "1. ClamAV 1.5.3 (GPL-2.0) — 受け入れる資料にウイルスが含まれていないかを確認する",
    "   ために同梱しています。常駐監視は行わず、利用者が指定したフォルダを、利用者の",
    "   操作に応じて一度だけ検査します。ウイルス対策ソフトウェアとしての機能",
    "   （常駐保護、システムの変更、他のセキュリティ製品との連携）は持ちません。",
    "   ウイルス定義データベースは同梱しておらず、利用者が設定画面から明示的に",
    "   取得したときにのみ database.clamav.net へ接続します。",
    "",
    "2. Siegfried 1.11.6 (Apache-2.0) — ファイル形式を PRONOM の識別子で判定するために",
    "   同梱しています。ネットワークには接続しません。",
    "",
    "いずれも改変していない公式ビルドです。ライセンスの表示とソースの入手方法は、",
    "パッケージに含まれる NOTICE ファイルに記載しています。",
    "",
    "runFullTrust を宣言しているのは、上記の同梱実行ファイルを子プロセスとして",
    "起動するためです。",
    "",
    "アプリは利用者の個人情報を収集・送信しません。すべての処理は端末内で完結します。",
