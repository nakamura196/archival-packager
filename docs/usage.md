...
変換でできた派生物は、**もう一度開いて読み戻します**。TIFF は Pillow で最後まで
展開し、PDF は pypdf で開き直して 1 ページ以上あることを確かめます。
読み戻せなかった派生物は捨て、その処理は失敗として記録します。
これはフォーマットの検証ではありません。開けたからといって、その形式の仕様に
合っているとは限りません。
